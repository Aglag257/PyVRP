...
import pyvrp.search_legacy as _legacy_search
...
from pyvrp.search_legacy import (
    NODE_OPERATORS,
    ROUTE_OPERATORS,
    LocalSearch,
    NeighbourhoodParams,
...
class SolveParams:
    ...
    def from_file(cls, loc: str | pathlib.Path):
        ...
        if "node_ops" in data:
            node_ops = [getattr(_legacy_search, op) for op in data["node_ops"]]

        route_ops = ROUTE_OPERATORS
        if "route_ops" in data:
            route_ops = [getattr(_legacy_search, op) for op in data["route_ops"]]
