...
from pyvrp.ProgressPrinterLegacy import ProgressPrinter
from pyvrp.Result import Result
from pyvrp.StatisticsLegacy import Statistics
...
class GeneticAlgorithm:
    ...
    def _improve_offspring(self, sol: Solution):
        ...
        if (
            ...
            booster = getattr(self._pm, "booster_cost_evaluator", None)
            if booster is None:
                booster = getattr(self._pm, "max_cost_evaluator", None)

            booster_eval = booster() if booster is not None else self._cost_evaluator
            sol = self._search(sol, booster_eval)
