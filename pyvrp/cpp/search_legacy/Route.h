#ifndef PYVRP_SEARCH_LEGACY_ROUTE_H
#define PYVRP_SEARCH_LEGACY_ROUTE_H
    ...
    auto const fn = [&](auto &&segment, auto &&...args)
    {
        ...
        auto const merge = [&](auto const &self, auto &&other, auto &&...args)
        {
            ...
            if (other.endsAtReloadDepot())
            {
                ...
                DurationSegment const depotSeg = {depot, depot.serviceDuration};
                ds = DurationSegment::merge(edgeDur, depotSeg, ds);
                ds = ds.finaliseFront();
...
std::ostream &operator<<(std::ostream &out, pyvrp::search_legacy::Route const &route);

std::ostream &operator<<(std::ostream &out,  // for debugging
                         pyvrp::search_legacy::Route::Node const &node);

template <>  // specialisation for pyvrp::search_legacy::Route
pyvrp::Cost
pyvrp::CostEvaluator::penalisedCost(
    pyvrp::search_legacy::Route const &route) const;

#endif  // PYVRP_SEARCH_LEGACY_ROUTE_H
