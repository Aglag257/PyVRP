...
bool Route::overlapsWith(Route const &other, double tolerance) const
{
    ...
    auto const thisAngle = std::atan2(tY.get() - dY, tX.get() - dX);
    auto const otherAngle = std::atan2(oY.get() - dY, oX.get() - dX);
...
template <>
pyvrp::Cost
pyvrp::CostEvaluator::penalisedCost(pyvrp::search_legacy::Route const &route)
    const
{
    if (route.empty())
        return 0;
